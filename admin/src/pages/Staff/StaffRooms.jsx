...
const StaffRooms = () => {
  ...
  const roomTypes = useMemo(() => {
    return Array.from(
      new Set((rooms || []).map((room) => room.room_type || room.type).filter(Boolean))
    ).sort((a, b) => a.localeCompare(b));
  }, [rooms]);
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-md animate-in fade-in duration-500">
        <div className="bg-white w-full max-w-4xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[90vh] relative border border-white/20">
          ...
          <div className="w-full md:w-[48%] p-8 flex flex-col">
            <div className="flex flex-col mb-6">
              <div className="flex justify-between items-center w-full mb-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={onClose}
                    className="p-2.5 bg-slate-50 hover:bg-red-50 hover:text-red-500 rounded-full transition-all group shrink-0"
                  >
            ...
            <div className="mt-8">
              <div className="w-full py-4 bg-slate-100 text-slate-500 rounded-2xl font-bold flex items-center justify-center gap-3">
                View only
              </div>
            </div>
          </div>
    ...
    <div className="p-6 bg-slate-50 min-h-screen font-sans text-slate-900">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start mb-5 gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">View listings and availability. Editing is restricted.</p>
        </div>
        ...
        <div className="flex flex-col items-end w-full md:w-auto">
          <div className="flex gap-3">
            <button
              ...
              className="p-3 bg-white border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-50 hover:text-black transition-all shadow-sm"
            >
              <RefreshCw size={20} className={loading ? "animate-spin" : ""} />
            </button>
            <div className="flex items-center gap-2 px-4 py-3 rounded-xl bg-emerald-50 border border-emerald-100 text-emerald-700 text-[11px] font-black uppercase tracking-widest">
              View only
            </div>
          </div>
        ...
        <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto items-center">
          <div className="relative w-full md:w-48">
            ...
            {activeDropdown === "building" && (
              <div className="absolute z-50 top-full left-0 w-full mt-2 bg-white border border-slate-200 rounded-2xl shadow-xl p-1 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                <div
                  onClick={() => { setFilterBuilding(""); setActiveDropdown(null); }}
                  className="px-4 py-2.5 text-xs font-bold rounded-lg cursor-pointer hover:bg-slate-100 text-slate-400 flex items-center gap-3"
                >
                ...
                {buildings.slice(0, showMoreBuildings ? buildings.length : 3).map((b) => (
                  <div
                    key={b}
                    onClick={() => { setFilterBuilding(b); setActiveDropdown(null); }}
                    className="px-4 py-3 text-xs font-bold rounded-lg cursor-pointer hover:bg-slate-100 text-slate-700 transition-colors"
                  >
                    {b}
                  </div>
                ))}
                {buildings.length > 3 && (
                  <button
                    type="button"
                    onClick={(e) => { e.stopPropagation(); setShowMoreBuildings(!showMoreBuildings); }}
                    className="w-full py-2 text-[10px] font-black uppercase text-slate-500 hover:bg-slate-100 border-t border-slate-50"
                  >
          ...
          <div className="relative w-full md:w-60">
            ...
            {activeDropdown === "type" && (
              <div className="absolute z-50 top-full left-0 w-full mt-2 bg-white border border-slate-200 rounded-2xl shadow-xl p-1 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                <div
                  onClick={() => { setFilterType(""); setActiveDropdown(null); }}
                  className="px-4 py-2.5 text-xs font-bold rounded-lg cursor-pointer hover:bg-slate-100 text-slate-400 flex items-center gap-3"
                >
                ...
                {roomTypes.slice(0, showMoreTypes ? roomTypes.length : 3).map((t) => (
                  <div
                    key={t}
                    onClick={() => { setFilterType(t); setActiveDropdown(null); }}
                    className="px-4 py-3 text-xs font-bold rounded-lg cursor-pointer hover:bg-slate-100 text-slate-700 transition-colors"
                  >
                    {t}
                  </div>
                ...
                {roomTypes.length > 3 && (
                  <button
                    type="button"
                    onClick={(e) => { e.stopPropagation(); setShowMoreTypes(!showMoreTypes); }}
                    className="w-full py-2 text-[10px] font-black uppercase text-slate-500 hover:bg-slate-100 border-t border-slate-50"
                  >
          ...
          <div className="relative w-full md:w-48">
            ...
            {activeDropdown === "status" && (
              <div className="absolute z-50 top-full left-0 w-full mt-2 bg-white border border-slate-200 rounded-2xl shadow-xl p-1 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                <div
                  onClick={() => { setFilterStatus(""); setActiveDropdown(null); }}
                  className="px-4 py-2.5 text-xs font-bold rounded-lg cursor-pointer hover:bg-slate-100 text-slate-400 flex items-center gap-3"
                >
                ...
                <div className="h-px bg-slate-100 my-1 mx-2" />
                <div
                  onClick={() => { setFilterStatus("Available"); setActiveDropdown(null); }}
                  className="px-4 py-3 text-xs font-bold rounded-lg cursor-pointer hover:bg-slate-100 text-slate-700 flex items-center justify-between transition-colors"
                >
                  <span>Available</span>
                ...
                <div
                  onClick={() => { setFilterStatus("Unavailable"); setActiveDropdown(null); }}
                  className="px-4 py-3 text-xs font-bold rounded-lg cursor-pointer hover:bg-slate-100 text-slate-700 flex items-center justify-between transition-colors"
                >
            ...
            <div className="p-5 flex-1 flex flex-col">
              ...
              <div className="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between">
                <label className="flex items-center gap-2 cursor-not-allowed opacity-70">
                  ...
                  <input type="checkbox" className="hidden" checked={room.available} readOnly />
