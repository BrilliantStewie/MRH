...
const AdminReviews = () => {
  const { backendUrl, postReviewReply, aToken } = useContext(AdminContext);
...
const [editText, setEditText] = useState("");
  ...
  const fetchReviews = async () => {
    try {
      // 1. Fetch ALL bookings (since reviews are embedded in them)
  ...
  const handleSendReply = async (bookingId) => {
    const message = replyText[bookingId];

    if (!message || message.trim() === "") {
      return toast.warning("Please type a response first.");
    }

    // Use the bookingId as the identifier for the reply
    const success = await postReviewReply(bookingId, message);

    if (success) {
      setReplyText({ ...replyText, [bookingId]: "" });
    ...
    {review.reviewChat.map((chat) => (
      <div
        ...
        className={`relative group rounded-lg p-5 text-sm transition ${
          chat.senderRole === "admin" || chat.senderRole === "staff"
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[10px] text-slate-400 ml-auto">
            ...
            {chat.isEdited && (
              <span className="ml-1 italic text-slate-400">(edited)</span>
            )}
          </span>
        </div>
        ...
        {editingReplyId === chat._id ? (
          <>
            ...
            <div className="flex gap-2 mt-2 justify-end">
              ...
              <button
                onClick={() => handleEditReply(review._id, chat._id)}
          ...
          <>
            <p className="text-slate-600 text-[13px]">{chat.message}</p>
                  ...
                  {!activeReplyId || activeReplyId !== review._id ? (
                      ...
                      <div className="animate-in fade-in slide-in-from-top-1 duration-200">
                          <textarea
                            ...
                            rows="3"
                            placeholder={`Reply to ${review.user_id?.firstName || "guest"}...`}
