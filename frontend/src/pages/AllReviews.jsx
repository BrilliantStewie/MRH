...
import { Star, Calendar, CornerDownRight } from "lucide-react";
        ...
        <div className="space-y-10">
          {isLoading ? (
            ...
            reviews.map((review) => (
              <div
                key={review._id}
                className="group relative bg-white rounded-[2rem] p-10 border border-gray-100 shadow-none transition-all duration-500 ease-in-out overflow-hidden
                           hover:shadow-[0_20px_25px_-5px_rgb(0_0_0_/_0.05),_0_8px_10px_-6px_rgb(0_0_0_/_0.05)]
                           before:absolute before:inset-y-0 before:left-0 before:w-[6px] before:bg-[#F6AD55] before:opacity-0 before:transition-opacity before:duration-500
              ...
              >
                ...
                <div className="flex justify-between items-start relative z-10">
                  <div className="flex items-center gap-5">
                    ...
                    {/* AVATAR LOGIC: Checks userModel 'image' field */}
                    <div className="shrink-0">
                      {review.userId?.image ? (
                        ...
                        <div className="w-14 h-14 rounded-full bg-[#1E293B] text-white flex items-center justify-center font-bold text-lg border-2 border-white shadow-sm">
                ...
                {review.response && (
                  <div className="mt-10 bg-[#F1F5F9]/50 rounded-[1.5rem] p-7 ml-6 border border-gray-50 relative z-10">
                    <div className="flex items-start gap-4">
                      <CornerDownRight
                        size={22}
                        className="text-[#F6AD55] mt-1 shrink-0"
                      />
                      <div className="w-full">
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          <span className="bg-[#F6AD55] text-[10px] font-black text-white px-2.5 py-1 rounded-md uppercase tracking-tighter">
                            Admin
                          </span>
                ...
                return (
                  <div key={num} className="flex items-center gap-5">
                    ...
                    <div className="flex-1 h-2.5 bg-slate-100 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-[#F6AD55] rounded-full transition-all duration-500"
                        style={{ width: `${percentage}%` }}
                      />
                    </div>
                    <span className="text-sm font-semibold text-slate-400 w-5 text-right">
                      {count}
                    </span>
                  </div>
                );
