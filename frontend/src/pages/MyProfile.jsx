...
import {
  ...
  UserCircle, Settings, CheckCircle, BadgeCheck, Info
} from "lucide-react";
  ...
  const updateUserProfileData = async () => {
    try {
      ...
      if (localEditData.newPassword) {
        if (localEditData.newPassword !== localEditData.confirmPassword) return toast.error("Verification failed: Passwords mismatch");
  ...
  return userData && (
    <div className="min-h-screen bg-slate-50/60 pt-24 pb-14 px-4 md:px-10">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
          ...
          <div className="px-8 pb-8 flex flex-col md:flex-row items-end gap-6 -mt-14">
            <div className="relative group shrink-0">
              <div className="w-32 h-32 rounded-full border-[6px] border-white bg-white shadow-lg overflow-hidden transition-transform group-hover:scale-105 duration-300">
                <img
            ...
            <div className="mb-2">
              {!isEdit ? (
                ...
                <div className="flex gap-3">
                  <button onClick={() => setIsEdit(false)} className="px-5 py-2.5 text-slate-500 font-bold text-xs uppercase tracking-widest hover:text-slate-800 transition-colors">Discard</button>
                  <button onClick={updateUserProfileData} className="px-7 py-2.5 bg-blue-600 text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-blue-700 shadow-md shadow-blue-100 active:scale-95 flex items-center gap-2">
        ...
        {/* --- ALIGNED BENTO GRID --- */}
        {/* Changed from 'items-start' to 'items-stretch' (default) so columns equal height */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white rounded-[2rem] shadow-sm border border-slate-100 p-8 h-full flex flex-col">
            ...
            {/* Inputs Container - Pushes content to fill space if needed */}
          ...
          {/* Added h-full and flex flex-col to force height match and space contents */}
