...
const VerifyPhoneOtp = ({ phone, onClose, onSuccess, backendUrl, token }) => {
  ...
  const handleResendOtp = async () => {
    ...
    try {
      const { data } = await axios.post(
        `${backendUrl}/api/user/send-phone-otp-update`,
        { phone },
        { headers: { token } }
      );
      if (data.success) {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        `${backendUrl}/api/user/verify-phone-otp-update`,
        { otp: otpCode },
        { headers: { token } }
      );
      ...
      <div className="relative bg-white w-full max-w-[420px] rounded-[24px] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-300">
        <div className="bg-zinc-900 px-8 py-6 flex items-center justify-between">
          <div className="flex items-center gap-3 text-white">
            ...
            <div>
              <h2 className="text-sm font-bold uppercase tracking-widest">Phone</h2>
        ...
        <div className="p-8 sm:p-10">
          {isSuccess ? (
            <div className="flex flex-col items-center py-4 text-center animate-in slide-in-from-bottom-2">
              ...
              <h3 className="text-xl font-bold text-zinc-900">Phone Verified</h3>
              <p className="text-zinc-500 mt-2 text-sm">Your number is confirmed.</p>
            </div>
          ) : (
            <>
              <div className="mb-8">
                ...
                <p className="text-zinc-500 mt-2 text-sm leading-relaxed">
                  Enter the code sent to <span className="text-zinc-900 font-semibold">{phone}</span>
                </p>
              </div>
