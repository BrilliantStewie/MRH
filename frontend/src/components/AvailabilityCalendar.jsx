...
const AvailabilityCalendar = ({ isOpen, onClose, bookings }) => {
  ...
  const getDayClass = (date) => {
    ...
    const isReserved = unavailableDates.some((reservedDate) => reservedDate.getTime() === time);
  ...
  return (
    /* Centered modal, reduced size, no internal scrolling. */
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
      ...
      <style>{`
        ...
        .pro-calendar-v4 .react-datepicker__day-name {
          ...
          font-weight: 700;
          font-size: 0.7rem;
          width: 2.5rem; /* Adjusted to prevent overlapping */
          margin: 0.25rem;
      ...
      <div className="bg-white w-full max-w-md rounded-[1.75rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.15)] overflow-hidden flex flex-col border border-slate-200">
        
        <div className="relative px-7 py-5 border-b border-slate-50 flex justify-between items-center bg-white">
          <button
            type="button"
            onClick={onClose}
            className="absolute right-6 top-5 inline-flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-50 hover:text-slate-900"
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.15em] mt-0.5">Live Schedule</p>
            </div>
          </div>
        </div>

        <div className="p-6 pro-calendar-v4 flex flex-col items-center">
          <DatePicker
            inline
            readOnly
              ...
              <div className="w-full flex items-center justify-between px-3 mb-6">
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    className="w-9 h-9 flex items-center justify-center rounded-full border border-slate-200 hover:border-slate-900 transition-all pointer-events-auto bg-white"
                  >
                    <ChevronLeft size={18} />
                  </button>
                  <button 
        ...
        <div className="px-7 py-4 bg-slate-50 border-t border-slate-100 flex items-center">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">Available</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-amber-400 border border-amber-500" />
              <span className="text-[10px] font-bold text-amber-700 uppercase tracking-wide">Reserved</span>
            </div>
          </div>
        </div>
      </div>
